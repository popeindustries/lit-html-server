...
const { isPromise, isStream, isSyncIterator } = require('./is.js');
    ...
    if (isAsyncValue) {
      ...
      if (attributeNameMatch !== null && attributeNameMatch[1] !== undefined) {
        ...
        if (
          ...
          // Remove header from nested template (or escape if not nested)
      ...
      // Ignore if undefined, otherwise remove header from nested template
      // (or escape if not nested)
